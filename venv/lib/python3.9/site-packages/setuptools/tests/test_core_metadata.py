...
@pytest.mark.parametrize(
    ("content", "result"),
    (
        pytest.param("Just a single line", None, id="single_line",),
        pytest.param(
            "Multiline\nText\nwithout\nextra indents\n", None, id="multiline",
        ),
        pytest.param(
...
def __read_test_cases():
    ...
    return [
        ...
        ("Metadata Version 1.2: Requires-Python", params(python_requires=">=3.7",),),
        pytest.param(
            "Metadata Version 1.2: Project-Url",
            params(project_urls=dict(Foo="https://example.bar")),
            marks=pytest.mark.xfail(reason="Issue #1578: project_urls not read",),
        ),
        (
        ...
        (
            "Missing author",
            dict(name="foo", version="1.0.0", author_email="snorri@sturluson.name",),
        ),
        (
            "Missing author e-mail",
            dict(name="foo", version="1.0.0", author="Snorri Sturluson",),
        ),
        ("Missing author and e-mail", dict(name="foo", version="1.0.0",),),
        ("Bypass normalized version", dict(name="foo", version=sic("1.0.0a"),),),
    ]
    ...
    def test_license_files_dynamic(self, extra_toml, tmpdir_cwd):
        ...
        files = {
            "pyproject.toml": self.STATIC_CONFIG["pyproject.toml"].replace(
                'license = "AGPL-3.0-or-later"', f"dynamic = ['license']\n{extra_toml}",
            ),
...
def _assert_roundtrip_message(metadata: str) -> None:
    ...
    serialization_policy = EmailPolicy(
        utf8=True, mangle_from_=False, max_line_length=0,
    )
