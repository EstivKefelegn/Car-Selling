...
@pytest.mark.parametrize(
    "files",
    [{**EXAMPLE, "setup.py": SETUP_SCRIPT_STUB}, EXAMPLE,],  # No setup.py script
)
...
class TestFinderTemplate:
    ...
    def test_packages(self, tmp_path):
        files = {
            "src1": {"pkg1": {"__init__.py": "", "subpkg": {"mod1.py": "a = 42"},},},
            "src2": {"mod2.py": "a = 43"},
        }
        jaraco.path.build(files, prefix=tmp_path)
    ...
    def test_no_recursion(self, tmp_path):
        # See issue #3550
        files = {
            "pkg": {"__init__.py": "from . import pkg",},
        }
        jaraco.path.build(files, prefix=tmp_path)
    ...
    def test_similar_name(self, tmp_path):
        files = {
            "foo": {"__init__.py": "", "bar": {"__init__.py": "",},},
        }
        jaraco.path.build(files, prefix=tmp_path)
    ...
    def test_case_sensitivity(self, tmp_path):
        files = {
            "foo": {
                "__init__.py": "",
                "lowercase.py": "x = 1",
                "bar": {"__init__.py": "", "lowercase.py": "x = 2",},
            },
        }
        jaraco.path.build(files, prefix=tmp_path)
        mapping = {
    ...
    def test_namespace_case_sensitivity(self, tmp_path):
        files = {
            "pkg": {
                "__init__.py": "a = 13",
                "foo": {"__init__.py": "b = 37", "bar.py": "c = 42",},
            },
        }
        jaraco.path.build(files, prefix=tmp_path)
...
class TestOverallBehaviour:
    ...
    EXAMPLES = {
        ...
        "custom-layout": {
            ...
            "MANIFEST.in": EXAMPLE["MANIFEST.in"],
            "otherfile.py": "",
            "mypkg": {"__init__.py": "", "mod1.py": FLAT_LAYOUT["mypkg"]["mod1.py"],},
