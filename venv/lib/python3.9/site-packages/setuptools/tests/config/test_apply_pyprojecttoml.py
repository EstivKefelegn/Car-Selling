...
def _pep621_example_project(
    tmp_path, readme="README.rst", pyproject_text=PEP621_EXAMPLE,
):
...
def test_utf8_maintainer_in_metadata(  # issue-3663
    expected_maintainers_meta_value, pyproject_text, tmp_path,
):
...
def test_license_in_metadata(
    license, license_expression, content_str, not_content_str, pyproject_text, tmp_path,
):
    pyproject = _pep621_example_project(
        tmp_path, "README", pyproject_text=pyproject_text,
    )
...
class TestLicenseFiles:
    ...
    def test_deprecated_file_expands_to_text(self, tmp_path):
        ...
        text = PEP621_EXAMPLE.replace(
            'license-files = ["LICENSE.txt"]', 'license = {file = "LICENSE.txt"}',
        )
