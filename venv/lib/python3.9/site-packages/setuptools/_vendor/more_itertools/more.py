    ...
    *too_long* will be called with `n + 1`.
    ...
    if too_short is None:
        too_short = lambda item_count: raise_(
            ValueError, "Too few items in iterable (got {})".format(item_count),
        )
...
def distinct_combinations(iterable, r):
    ...
    while generators:
        ...
        if level + 1 == r:
            ...
            generators.append(
                unique_everseen(
                    enumerate(pool[cur_idx + 1 :], cur_idx + 1), key=itemgetter(1),
                )
            )
            level += 1
...
def outer_product(func, xs, ys, *args, **kwargs):
    ...
    ys = tuple(ys)
    return batched(
        starmap(lambda x, y: func(x, y, *args, **kwargs), product(xs, ys)), n=len(ys),
    )
