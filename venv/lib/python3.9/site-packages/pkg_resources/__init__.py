    ...
    (
        (
            vendor_path := os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "setuptools", "_vendor"
            )
        )
        not in sys.path
    )
...
class Environment:
    ...
    @overload
    def obtain(
        self, requirement: Requirement, installer: _InstallerType | None = None,
    ) -> Distribution | None:
        ...
...
class EntryPoint:
    ...
    def require(
        self, env: Environment | None = None, installer: _InstallerType | None = None,
    ) -> None:
        if not self.dist:
    ...
    @classmethod
    def parse_group(
        cls, group: str, lines: _NestedStr, dist: Distribution | None = None,
    ) -> dict[str, Self]:
...
class Distribution:
    ...
    def insert_on(  # noqa: C901
        self, path: list[str], loc=None, replace: bool = False,
    ) -> None:
...
def _initialize_master_working_set() -> None:
    ...
    add_activation_listener(
        lambda dist: dist.activate(replace=True), existing=False,
    )
    working_set.entries = []
    # match order
