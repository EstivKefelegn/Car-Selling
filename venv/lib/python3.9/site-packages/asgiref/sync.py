...
class SyncToAsync(Generic[_P, _R]):
    ...
    async def __call__(self, *args: _P.args, **kwargs: _P.kwargs) -> _R:
        ...
        exec_coro = loop.run_in_executor(
            executor,
            functools.partial(
                self.thread_handler, loop, sys.exc_info(), task_context, func, child,
            ),
        )
        ret: _R
        try:
...
def async_to_sync(
    ...
    *,
    ...
    if awaitable is None:
        return lambda f: AsyncToSync(f, force_new_loop=force_new_loop,)
    ...
    *,
    ...
    if func is None:
        return lambda f: SyncToAsync(
            f, thread_sensitive=thread_sensitive, executor=executor, context=context,
        )
    return SyncToAsync(
        func, thread_sensitive=thread_sensitive, executor=executor, context=context,
    )
